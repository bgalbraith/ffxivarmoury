...
for char in chars:
    ...
    c = pycurl.Curl()
    b = StringIO.StringIO()
    c.setopt(pycurl.URL, "http://lodestone.finalfantasyxiv.com/rc/character/playlog?cicuid=%d&num=1000&p=1" % char.cicuid)
    ...
    for group in events.values():
        for event in group:
            try:
                event = str(event)
                if event in ["Hatching-tide reward obtained!","Firefall Faire reward obtained!"]:
